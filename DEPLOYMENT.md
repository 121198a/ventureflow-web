...
| **Frontend & API (Container/VPS)** | **Render** or **Koyeb** | Free web service tier | Hostinger VPS / Hetzner ($2.50–$4/mo) |
...
| **Total Monthly Cost** | **$0.00 / month** | Suitable for testing, portfolio & moderate traffic | **~$3.00 / month** (Hostinger VPS / Linux VPS) |
...
3. **Cross-Platform Client IP Detection**:
   `lib/rate-limit.ts` inspects `cf-connecting-ip` (Cloudflare), `x-real-ip`, and `x-forwarded-for` (Vercel, Netlify, Nginx, Hostinger), ensuring rate limiting works consistently everywhere.
...
### Option D: Free Cloud Containers (Render / Railway / Koyeb)
